...
class TestQueueEntryListView:
    ...
    def test_create_entry_authenticated(self, authenticated_client, room):
        data = {
            'room': room.id,
            'status': 'waiting'
        }
        response = authenticated_client.post(reverse('entry-list'), data)
        assert response.status_code == status.HTTP_201_CREATED
        assert QueueEntry.objects.count() >= 1

    def test_create_entry_unauthenticated(self, api_client, room):
        data = {
            'room': room.id,
            'status': 'waiting'
        }
        response = api_client.post(reverse('entry-list'), data)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_create_entry_invalid_room(self, authenticated_client):
        data = {
            'room': 99999,
            'status': 'waiting'
        }
    ...
    def test_position_assignment(self, authenticated_client, room):
        data = {'room': room.id, 'status': 'waiting'}
        response = authenticated_client.post(reverse('entry-list'), data)
        assert response.status_code == status.HTTP_201_CREATED
...
class TestQueueEntryDetailView:
    ...
    def test_get_entry_detail_authenticated(self, authenticated_client, queue_entry):
        response = authenticated_client.get(
            reverse('entry-detail', kwargs={'pk': queue_entry.id})
        )
        assert response.status_code == status.HTTP_200_OK
